...
import static azmalent.terraincognita.core.registry.ModBlocks.*;
...
public class TIBlockStateProvider extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        ...
        Block pottedCactusFlower = ForgeRegistries.BLOCKS.getValue(TerraIncognita.prefix("potted_cactus_flower"));
        simpleBlock(pottedCactusFlower, new UncheckedModelFile(TerraIncognita.prefix("block/potted_cactus_flower")));

        blockItem(SMOOTH_CACTUS.get());

        flowerPot(SWAMP_REEDS.get(), true);
        ...
        SWEET_PEAS.forEach(sweetPea -> {
//            var uncutModel = models().getBuilder(name(sweetPea.get()))
//                .parent(new UncheckedModelFile("block/vine"))
//                .texture("vine", blockTexture(sweetPea.get()))
//                .texture("particle", blockTexture(sweetPea.get()));
//
//            var cutModel = models().getBuilder(name(sweetPea.get()) + "_cut")
//                .parent(new UncheckedModelFile("block/vine"))
//                .texture("vine", suffix(blockTexture(sweetPea.get()), "_cut"))
//                .texture("particle", suffix(blockTexture(sweetPea.get()), "_cut"));



            generatedItem(sweetPea, "block", name(sweetPea.get()));
        });
        ...
        horizontalBlock(BASKET.get(), new UncheckedModelFile(TerraIncognita.prefix("block/basket")));
    ...
    private void woodType(TIWoodType type) {
        ...
        leaves(type.LEAVES.get(), type.LEAF_CARPET.get(), type.LEAF_PILE.get());
    ...
    private void leaves(LeavesBlock leaves, TILeafCarpetBlock leafCarpet, TILeafPileBlock leafPile) {
    ...
    private void leafPile(LeavesBlock leaves, TILeafPileBlock leafPile) {
        ModelFile leafPileModel = models().getBuilder(name(leafPile)).parent(new UncheckedModelFile(TerraIncognita.prefix("block/leaf_pile"))).texture("all", blockTexture(leaves));
        ...
        generatedItem(leafPile, "block", name(leaves));
    }
