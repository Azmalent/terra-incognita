...
public class ModBiomes {
    ...
    public static void registerBiomes() {
        if (TIConfig.Biomes.snowlessTundraWeight.get() > 0) {
            registerBiome(BiomeManager.BiomeType.ICY, TUNDRA, TIConfig.Biomes.snowlessTundraWeight.get());
            BiomeDictionary.addTypes(BiomeUtil.getBiomeKey(TUNDRA.getId()), Type.COLD, Type.WASTELAND, Type.OVERWORLD);
        }

        if (TIConfig.Biomes.rockyTundraWeight.get() > 0) {
            registerBiome(BiomeManager.BiomeType.ICY, ROCKY_TUNDRA, TIConfig.Biomes.rockyTundraWeight.get());
